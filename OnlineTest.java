...
class OnlineTest extends JFrame implements ActionListener {
	...
	OnlineTest(String s) {
		...
		btnBookmark.setBackground(Color.CYAN);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("Result")) {
			if (check())
				count = count + 1;
			current++;
			JOptionPane.showMessageDialog(this, "Your Test is Completed-->correct answers= " + count);
			System.exit(0);
		}
	}
	...
	void set() {
		...
		if (current == 0) {
			...
			radioButton[3].setText("Lambda Exp");
		}
		if (current == 1) {
		...
		for (int i = 0, j = 0; i <= 90; i += 30, j++)
			radioButton[j].setBounds(50, 80 + i, 300, 20);
	}
